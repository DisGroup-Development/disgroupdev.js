const { BaseGuildTextChannel, Client, ColorResolvable, EmojiIdentifierResolvable, MessageMentionOptions, MessageReaction, Snowflake, User, resolveColor, resolvePartialEmoji } = require('discord.js');
...
class GiveawayManager extends EventEmitter {
    ...
    _generateGiveawayEndEmbed(giveaway, winnerIds) {
        ...
        for(const i = 1; giveawayMessagesDescription(winners).length > 4096 || giveawayMessages.title.length + giveawayMessages.endedAt.length + giveawayMessagesDescription(winners).length > 6000; i++) {
