...
class GiveawayManager extends EventEmitter {
                    ...
                    if(giveawayWinnerMessageFirstPart.length > 0) {
                        ...
                        giveaway.message.channel.send({
                            ...
                            reply: {
                                ...
                                messageReference: giveaway.message.id
                            }
                        });
    ...
    rollGiveaway(giveaway) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            let giveawayGuild = await this.client.guilds.fetch(giveaway.guildId).catch(() => {}) ?? giveaway.message.guild;

            giveaway.message = await giveaway.fetchMessage().catch(() => {}) ?? giveaway.message;

            await giveawayGuild.members.fetch().catch(() => {});
            ...
            const finalGiveawayWinners = giveawayWinners.map(async (winner) => await giveawayGuild.members.fetch(winner.id).catch(() => {}));
