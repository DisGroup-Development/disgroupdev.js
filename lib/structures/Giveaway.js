const { Client, Collection, ColorResolvable, EmojiIdentifierResolvable, Message, MessageMentionOptions, MessageReaction, Snowflake, User, Util } = require('discord.js');
...
class Giveaway {
    ...
    _fillInEmbed(emb) {
        ...
        if(emb.data.fields?.length && emb.data.fields?.length > 0) {
            emb.spliceFields(
                0,
    ...
    checkWinnerEntry(user) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            const member = await this.message?.guild.members.fetch(user.id).catch((e) => reject(new DisGroupDevError(e)));
    ...
    get embedColor() {
        ...
        return this.data.embedColor ?? Util.resolveColor('#3496eb');
    ...
    get embedColorEnded() {
        ...
        return this.data.embedColorEnded ?? Util.resolveColor('#ffde22');
    ...
    fetchAllEntries() {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            const giveawayMessage = await this.fetchMessage().catch((e) => reject(new DisGroupDevError(e)));
            ...
            let giveawayUsers = await giveawayReaction.users.fetch().catch((e) => reject(new DisGroupDevError(e)));
    ...
    fetchMessage() {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            const giveawayMessage = await giveawayChannel.messages.fetch(this.messageId).catch((err) => { if(err.code === 10008) tryAgainLater = false });
    ...
    get lastChance() {
        ...
        return {
            ...
            embedColor: this.data.lastChance.embedColor ?? Util.resolveColor('#fcb22d'),
    ...
    get messageReaction() {
        ...
        const reactionEmoji = Util.resolvePartialEmoji(this.reaction);
    ...
    get messages() {
        ...
        return {
            ...
            drop: this.data.messages.drop ?? 'Be the first who reacts with {this.reaction} to win!',
            endedAt: this.data.messages.endedAt ?? 'Ended at',
            footer: this.data.messages.footer ?? {
                iconURL: null,
                text: 'Made with 💙 by DisGroup Development'
            },
            ...
            winners: this.data.messages.winners ?? 'Winner(s)'
    ...
    get pauseOptions() {
        ...
        return {
            ...
            embedColor: this.data.pauseOptions.embedColor ?? Util.resolveColor('#fe6b61'),
    ...
    get reaction() {
        ...
        if(!this.data.reaction && this.message) {
            ...
            const emoji = Util.resolvePartialEmoji(this.manager.options.emoji ?? '🎉');
        ...
        return Util.resolvePartialEmoji(this.data.reaction)?.name ?? '🎉';
